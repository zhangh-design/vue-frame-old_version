...
TjUI.form.tools.FileBox = TjUI.extend(js.base.fn,{
    ...
    methods: {
        onSuccess(response, file, fileList){
            ...
            ('on-success' in this.listeners) && this.listeners['on-success'](response, file, fileList)
        },
        onError(err, file, fileList){
            this.$message({type: 'warning',message: '上传失败!'});
            ('on-error' in this.listeners) && this.listeners['on-error'](err, file, fileList)
        },
        onExceed(files, fileList){
            this.$message({type: 'warning',message: '文件超出个数限制!'});
            ('on-exceed' in this.listeners) && this.listeners['on-exceed'](files, fileList)
        }
    },
    render(h){
        ...
        if(!this.isRender){
            ...
            let solts = []
            if(this.isShowTip){
                solts.push(h('a',{style: {'font-size': '12px',},attrs: {href: 'javascript: void(0);'},domProps: {innerHTML: this.tipText},})
                )
            }
            return h(
                'el-upload',
