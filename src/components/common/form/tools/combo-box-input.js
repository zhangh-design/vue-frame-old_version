	...
	methods: {
		...
		initLoadStore(){
			this.$api[this.conurl](this.queryParams).then(resData => {
                setTimeout(() => {
					this.curOptions.splice(0,this.curOptions.length,...((!!this.loadFilter) ? this.loadFilter(resData.data) : resData.data.map(item=>({[this.displayField]: item[this.displayField]+''}))))
				}, 0);
			}).catch( error =>{
				console.log(error)
            })
	...
	render(h){
		...
		let selectComponent = h(
			'el-select',
			{
				...
				class: 'user-select',
                props: {
                    value: this.vSelectValue,
                    multiple: false,
                },
                on: {
                    input: (val)=>{
                        let label = this.curOptions.find(option=> option[this.displayField]===val)[this.displayField]
                        this.$refs['combo-box-input'].setTextValue(label)
					},
					change: (val)=>{
						let name = this.curOptions.find(option=>option[this.displayField]===val)[this.displayField]
						this.vValue = name
						this.vSelectValue = name
						this.$emit('selectInputChange',name)
						if('change' in this.listeners){
							this.listeners.change(name)
						}
					}
                },
