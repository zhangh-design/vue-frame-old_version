...
TjUI.form.tools.ComboGrid = TjUI.extend(js.base.fn,{
    ...
    methods: {
                ...
                'grid',
                {
                    ...
                    on: {
                        selectRow: (selection, row) => {
                            ...
                            this.vValue = this.allSelectValue.map(selectValue => selectValue[this.displayField])
        ...
        loadFilter(){
            ...
            let defaultSelectRows = [],selectRows = []
            if(this.getLinkComponent(this.gridLink).getTotalPages() === 1){
                selectRows = []
                for (let key of this.vdefaultCheckedKeys) {
                    let row = (this.getLinkComponent(this.gridLink).getStore()).find((value, index, arr)=> value[this.valueField] == key)
                    if(row !== void(0)){
                        selectRows.push(row)
                    }
                }
            ...
            if(this.getLinkComponent(this.gridLink).getTotalPages() > 1){
                for (let [index, elem] of this.vdefaultCheckedKeys.entries()) {
                    selectRows.push({[this.valueField]: elem, [this.displayField]: this.vValue[index]})
                }
                let defaultSelectRows = this.getLinkComponent(this.gridLink).getStore().filter(row => this.vdefaultCheckedKeys.includes(row[this.valueField]))
                if(defaultSelectRows.length){
                    this.getLinkComponent(this.gridLink).toggleSelection(defaultSelectRows)
                }
            }
            if(selectRows.length){
                selectRows.forEach(defaultRow => {
