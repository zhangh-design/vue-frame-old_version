...
TjUI.grid.table.Table = TjUI.extend(js.base.fn,{
    ...
    methods: {
        ...
        init(){
            ...
            let tbarHeight = 0
            if(this.getPanel.getTbar().length){
                for (let elem of this.getPanel.getTbar().values()) {
                    tbarHeight = tbarHeight + parseInt(elem.$el.offsetHeight)
                }
            }
            let paginationHeight = 32
            if(!this.getPanel.isPagination){
                paginationHeight = 0
            }
            this.tableInnerHeight = (parseInt(this.getPanel.$el.offsetHeight)-tbarHeight-paginationHeight)+'px'
        }
    },
    render(h){
        //构建列
        ...
        return h(
                    'el-table',
                    {
                        ...
                        props: {
                            ...
                            height: this.tableInnerHeight,         //String类型height值会转换为style的height
