    ...
    methods: {
                ...
                this.$api[this.conurl]({...this.queryParams,...{id: node.node.key}}).then(resList => {
                    ...
                    if(node.node.level === 1){
                        resList.data.forEach((element ,index) => {
                            let node = {label: element.node[this.displayField],id: element.node[this.valueField],nodeIndex: index,superIndex: index,indexs: [index],node: {...element.node}}
                            resData.push(node)
                        })
                    }else{
        ...
        getElementNode( resData,nodeData ){
            ...
            return dataNode.map((element, index) => {
                return {label: element.node[this.displayField],id: element.node[this.valueField],leaf: !(element.childs.length) ? true : false,nodeIndex: index,superIndex: nodeData.superIndex,indexs: [...nodeData.indexs,index],node: {...element.node}}
            })
        },
        //节点被点击时的回调
        ...
        currentChange(record, node){
            ...
            if(!this.showCheckbox){
                //触发v-model input事件
                this.$emit('treeChange',[{label: node.label,value: node.key}])
            }
        },
        //当某一节点被鼠标右键点击时会触发该事件
