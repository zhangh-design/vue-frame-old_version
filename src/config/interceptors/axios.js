...
import { isEmptyObject,uniqueArray } from '@/utils/tools'
...
export function requestSuccessFunc(requestConfig){
    ...
    //URL参数拦截认证
    if('validator' in requestConfig && !isEmptyObject(requestConfig.validator)){
        let params = {}
        ...
        if(!isEmptyObject(params)){
            ...
            if(code.status){
                GLOBAL.$vbus.emit('axios.request_error',(uniqueArray(code.msg)).join(' '))
                return Promise.reject('url params validator invalide')
            }
        }
    }
    //restful参数拦截认证
    if('restfulValidator' in requestConfig && !isEmptyObject(requestConfig.restfulValidator)){
        if(!isEmptyObject(requestConfig.restful_params)){
            let code = validate(requestConfig.restful_params,requestConfig['restfulValidator'])
            if(code.status){
                GLOBAL.$vbus.emit('axios.request_error',(uniqueArray(code.msg)).join(' '))
                return Promise.reject('url restful params validator invalide')
            }
        }
    }
