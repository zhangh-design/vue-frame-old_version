...
class MakeApi {
    ...
    _createApiSingle({
        ...
        config.forEach(api => {
            ...
            const { name,method,desc,path,mockPath,restful,headers={},params,validator={},restfulValidator={} } = api
            ...
            Object.defineProperty(this.api,apiname,{
                value(outParams,outOptions){
                    ...
                    return axios(MakeApi._requestStandard(_assign(requestOptions,outOptions),_data,validator,restfulValidator,restful,requestHeaders))
                }
            })
        });
    }
    ...
    static _requestStandard(options,data,validator,restfulValidator,restful,requestHeaders){
        //特殊字符过滤
        if(!_isEmpty(data)){
            _forIn(data,(value, key)=>{
		...
		(!_isEmpty(restfulValidator)) && (options.restfulValidator = restfulValidator)
        //restful风格检测
