...
const Detail = {
    ...
    data() {
        return {
            conurl: 'expense/travel/superior/doSubmitSuperior',
            rejectConurl: 'expense/travel/superior/doRejectSuperior',
            detailConurl: 'expense/travel/superior/getSuperiorDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/travel/superior',
                    dataconurl: 'expense/travel/superior/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `travel/superior/detail-edit-${this.record.id}-process`},processPanel)
            }else{
