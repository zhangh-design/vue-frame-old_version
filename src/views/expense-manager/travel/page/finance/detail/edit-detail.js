...
import pageDetail from "../../apply/detail/page/project/add-grid";
...
const Detail = {
    ...
    data() {
        return {
            conurl: 'expense/travel/finance/doSubmitFinance',
            rejectConurl: 'expense/travel/finance/doRejectFinance',
            detailConurl: 'expense/travel/finance/getFinanceDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/travel/finance',
                    dataconurl: 'expense/travel/finance/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `travel/finance/detail-edit-${this.record.id}-process`},processPanel)
            }else{
