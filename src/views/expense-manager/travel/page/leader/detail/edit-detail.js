...
const Detail = {
    ...
    data() {
        return {
            conurl: 'expense/travel/leader/doSubmitLeader',
            rejectConurl: 'expense/travel/leader/doRejectLeader',
            detailConurl: 'expense/travel/leader/getLeaderDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/travel/leader',
                    dataconurl: 'expense/travel/leader/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `travel/leader/detail-edit-${this.record.id}-process`},processPanel)
            }else{
