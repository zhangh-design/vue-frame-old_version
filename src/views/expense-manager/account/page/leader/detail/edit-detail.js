...
const Detail = {
    ...
    data(){
        return {
            conurl: 'expense/account/leader/doSubmitLeader',
            rejectConurl: 'expense/account/leader/doRejectLeader',
            detailConurl: 'expense/account/leader/getLeaderDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/account/leader',
                    dataconurl: 'expense/account/leader/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `account/leader/detail-edit-${this.record.id}-process`},processPanel)
            }else{
