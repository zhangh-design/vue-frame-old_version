...
const Detail = {
    ...
    data(){
        return {
            conurl: 'expense/account/teller/doSubmitTeller',
            rejectConurl: 'expense/account/teller/doRejectTeller',
            detailConurl: 'expense/account/teller/getTellerDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/account/teller',
                    dataconurl: 'expense/account/teller/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `account/teller/detail-edit-${this.record.id}-process`},processPanel)
            }else{
