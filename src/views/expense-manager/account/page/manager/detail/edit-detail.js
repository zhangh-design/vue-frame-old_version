...
const Detail = {
    ...
    data(){
        return {
            conurl: 'expense/account/manager/doSubmitManager',
            rejectConurl: 'expense/account/manager/doRejectManager',
            detailConurl: 'expense/account/manager/getManagerDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/account/manager',
                    dataconurl: 'expense/account/manager/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `account/manager/detail-edit-${this.record.id}-process`},processPanel)
            }else{
