...
const Detail = {
    ...
    data(){
        return {
            conurl: 'expense/account/finance/doSubmitFinance',
            rejectConurl: 'expense/account/finance/doRejectFinance',
            detailConurl: 'expense/account/finance/getfinanceDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'expense/account/finance',
                    dataconurl: 'expense/account/finance/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `account/finance/detail-edit-${this.record.id}-process`},processPanel)
            }else{
