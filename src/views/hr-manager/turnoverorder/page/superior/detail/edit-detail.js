...
const Detail = {
    ...
    data(){
        return {
            conurl: 'hr/turnoverorder/superior/doSubmitSuperior',
            rejectConurl: 'hr/turnoverorder/superior/doRejectSuperior',
            saveConurl:'hr/turnoverorder/superior/doSaveSuperior',
            detailConurl: 'hr/turnoverorder/superior/getSuperiorDetail',
            queryParams: {
            ...
            buttonsLayout: 'top',
            formButtons: [
                {text: '保存', authority: ['write'],link: `link-turnoverorderSuperior-savebtn-${this._uid}`, listeners: {click: this.doSave}},
    ...
    methods: {
        initDetailData(){
            this.detailData = [
                ...
                {span: 3,name: 'comment',type: 'TextArea',rows: 6,label: '主管评语',rule:[{required: true,message: '请输入主管评语', trigger: 'blur'}]},
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/turnoverorder/superior',
                    dataconurl: 'hr/turnoverorder/superior/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `hr/turnoverorder/superior/detail-edit-${this.record.id}-process`},processPanel)
            }else{
