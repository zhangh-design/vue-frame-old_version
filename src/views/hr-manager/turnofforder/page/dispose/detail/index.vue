<template>
	<div style="height:100%;overflow: hidden;">
		<el-row style="height:33px;background-color: #F3F3F3;" class="item-btn-cls">
			...
			<el-button type="primary" ref='submit-btn-ref' size="small" @click="doSubmit" :disabled="submitDisabled">提交</el-button>
		...
		<div :style="{height: formHeight+'px',overflow: 'auto'}">
			<el-form :ref="ref" :model="form" :rules="rules" class="turnofforder-apply-form-cls">
				...
				<el-row>
					<span style="margin-left:10px;color: #606266;margin-left:10px;">离职原因</span>
				</el-row>
				<el-row>
				...
				<el-row style="padding-left:10px;margin-top:20px;margin-bottom:20px;background-color: #F3F3F3">
					<table>
						<tr>
							...
							<td>
								<div style="width:140px">
									<el-form-item prop="workshift" style="margin-bottom: 0px;">
										<el-radio-group v-model="form.workshift" :disabled="workshiftDisabled">
							...
							<td class="gzjj-td-cls">
								<div style="width:360px">
									<el-form-item label="交接人" prop="workshiftname" label-width="120px">
										<el-input v-model="form.workshiftname" :disabled="workshiftnameDisabled" class="base-input-cls" placeholder="交接人"></el-input>
									</el-form-item>
									<el-form-item label="工作截止时间" prop="workshifttime" label-width="120px">
										<el-date-picker value-format="yyyy-MM-dd" :disabled="workshifttimeDisabled" v-model="form.workshifttime" class="base-input-cls" placeholder="工作截止时间"></el-date-picker>
									</el-form-item>
								</div>
							</td>
							...
							<td style="width: 100%;">
								<el-input type="textarea" rows=4 v-model="form.workinfo" :disabled="workinfoDisabled" placeholder="交接情况" maxlength=100></el-input>
							</td>
						</tr>
					</table>
				</el-row>
...
export default {
	name: 'turnofforder-dispose-edit-detail',
	...
	data(){
		return {
			...
			saveDisabled: false,
			workshiftDisabled: false,
			workshiftnameDisabled: false,
			workshifttimeDisabled: false,
			workinfoDisabled: false,
			lastFormDetail: {},
			saveConurl: 'hr/turnofforder/dispose/saveDispose',
			submitConurl: 'hr/turnofforder/dispose/submitDispose',
			rejectConurl: 'hr/turnofforder/dispose/rejectDispose',
			authorityConurl: 'user/getModuleAccess',
			authorityParams: {moduleid: this.moduleid,token: this.$store.getters['user/getToken']},
			detailConurl: 'hr/turnofforder/dispose/getDetail',
			...
			rules: {
				workshift: [
					{ required: true, message: '请选择交接情况', trigger: 'blur' }
				],
				workshiftname: [
					{ required: true, message: '请输入交接人', trigger: 'blur' }
				],
				workshifttime: [
					{ required: true, message: '请输入交接完成时间', trigger: 'blur' }
				],
			}
		}
	},
	...
	mounted(){
		...
		setTimeout(() => {
			...
			this.$api[this.authorityConurl](this.authorityParams).then(resData=>{
				...
				this.workshiftDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.workshiftnameDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.workshifttimeDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.workinfoDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
			})
		}, 0);
	},
	methods: {
		readDetail(){
			if(this.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/turnofforder/dispose',
                    dataconurl: 'hr/turnofforder/dispose/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `turnofforder/dispose/detail-edit-${this.row.id}-process`},processPanel)
            }else{
