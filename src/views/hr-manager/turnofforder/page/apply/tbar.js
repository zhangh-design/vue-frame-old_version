...
import detailPanel from './detail/add-detail'
...
const Tbar = {
    ...
    methods: {
        doAdd(){
            ...
            this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '添加',name: `turnofforder/apply/detail-add`},detailPanel)
        },
        doDelete(){
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                this.$api['hr/turnofforder/apply/deleteApply']({processid: this.record.process.id,token: this.$store.getters['user/getToken']}).then(resData=>{
        ...
        readDetail(){
            ...
            if(this.record.process.processcode!=null){
                ...
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `turnofforder/apply/detail-edit-${this.record.id}-process`},processPanel)
            }else{
