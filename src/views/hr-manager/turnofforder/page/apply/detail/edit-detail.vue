			...
			<el-form :ref="ref" :model="form" :rules="rules" class="turnofforder-apply-form-cls">
				<el-row style="padding-top:5px;">
					...
					<el-col :span="1" style="width: 290px">
						<el-form-item label="联系电话" prop="phone" label-width="80px">
							<el-input v-model="form.phone" :disabled="phoneDisabled" class="base-input-cls" placeholder="联系电话"></el-input>
						</el-form-item>
					</el-col>
					<el-col :span="1" style="width: 290px">
				...
				<el-row>
					<el-form-item prop="reason" label-width="10px">
						<el-input type="textarea" rows=4 v-model="form.reason" :disabled="reasonDisabled" placeholder="离职原因" maxlength=100></el-input>
					</el-form-item>
				</el-row>
...
export default {
	name: 'turnofforder-apply-edit-detail',
	...
	data(){
		return {
			...
			detailConurl: 'hr/turnofforder/apply/getDetail',
	...
	mounted(){
		...
		setTimeout(() => {
			...
			this.$api[this.authorityConurl](this.authorityParams).then(resData=>{
				...
				this.phoneDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.reasonDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
			})
		}, 0);
	},
	methods: {
		readDetail(){
			if(this.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/turnofforder/apply',
                    dataconurl: 'hr/turnofforder/apply/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `turnofforder/apply/detail-edit-${this.row.id}-process`},processPanel)
            }else{
