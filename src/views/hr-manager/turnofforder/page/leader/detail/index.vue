...
export default {
	name: 'turnofforder-leader-check-detail',
	props: ['mainPanel','mainGrid','window','type','row','tabName','moduleid'],
	data(){
		return {
			formHeight: 0,
			ref: 'turnofforder-form',
			conurl: 'hr/turnofforder/leader/submitLeader',
			rejectConurl: 'hr/turnofforder/leader/rejectLeader',
			detailConurl: 'hr/turnofforder/leader/getDetail',
		...
		readDetail(){
			if(this.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/turnofforder/leader',
                    dataconurl: 'hr/turnofforder/leader/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `turnofforder/leader/detail-edit-${this.row.id}-process`},processPanel)
            }else{
