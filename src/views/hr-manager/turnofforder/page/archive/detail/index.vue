							...
							<td class="gzjj-td-cls">
								<div style="width:360px">
									<el-form-item label="处理时间" prop="consttime" label-width="120px">
										<el-date-picker value-format="yyyy-MM-dd" disabled v-model="form.consttime" class="base-input-cls" placeholder="工作截止时间"></el-date-picker>
									</el-form-item>
								</div>
							</td>
							<td style="width:16px;padding: 0px 10px 0px 10px">
				...
				<el-row style="padding-left:10px;background-color: #F3F3F3">
					<table>
						<tr>
							...
							<td>
								<div style="width:140px">
									<el-form-item prop="prove" style="margin-bottom: 0px;">
										<el-radio-group v-model="form.prove" :disabled="proveDisabled">
							...
							<td class="gzjj-td-cls">
								<div style="width:360px">
									<el-form-item label="处理时间" prop="provetime" label-width="120px">
										<el-date-picker value-format="yyyy-MM-dd" :disabled="provetimeDisabled" v-model="form.provetime" class="base-input-cls" placeholder="工作截止时间"></el-date-picker>
									</el-form-item>
								</div>
							</td>
							<td style="width:16px;padding: 0px 10px 0px 10px">
							...
							<td style="width: 100%;">
								<el-input type="textarea" rows=4 v-model="form.proveinfo" :disabled="proveinfoDisabled" placeholder="情况说明" maxlength=100></el-input>
							</td>
						</tr>
					</table>
				</el-row>
...
export default {
	name: 'turnofforder-archive-edit-detail',
	...
	data(){
		return {
			...
			proveDisabled: false,
			provetimeDisabled: false,
			proveinfoDisabled: false,
			lastFormDetail: {},
			saveConurl: 'hr/turnofforder/archive/saveArchive',
			submitConurl: 'hr/turnofforder/archive/submitArchive',
			rejectConurl: 'hr/turnofforder/archive/rejectArchive',
			authorityConurl: 'user/getModuleAccess',
			authorityParams: {moduleid: this.moduleid,token: this.$store.getters['user/getToken']},
			detailConurl: 'hr/turnofforder/archive/getDetail',
			...
			form: {
				personname: this.row.personname,
				phone: this.row.phone,
				deptname: this.row.deptname,
				begintime: this.row.begintime,
				station: this.row.station,
				reason: this.row.reason,
				workshift: this.row.workshift !==null ? this.row.workshift+'' : this.row.workshift,
				workshiftname: this.row.workshiftname,
				workshifttime: this.row.workshifttime,
				workinfo: this.row.workinfo,
				deviceshift: this.row.deviceshift !==null ? this.row.deviceshift+'' : this.row.deviceshift,
				deviceinfo: this.row.deviceinfo,
				devicetime: this.row.devicetime,
				finance: this.row.finance !==null ? this.row.finance+'' : this.row.finance,
				consttime: this.row.consttime,
				financeinfo: this.row.financeinfo,
				prove: this.row.prove !==null ? this.row.prove+'' : this.row.prove,
				provetime: this.row.provetime,
				proveinfo: this.row.proveinfo,
			},
			process: {},
			rules: {
				prove: [
					{ required: true, message: '请选择离职证明', trigger: 'blur' }
				],
				provetime: [
	...
	mounted(){
		...
		setTimeout(() => {
			...
			this.$api[this.authorityConurl](this.authorityParams).then(resData=>{
				...
				this.proveDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.provetimeDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.proveinfoDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
			})
		}, 0);
	},
	methods: {
		readDetail(){
			if(this.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/turnofforder/archive',
                    dataconurl: 'hr/turnofforder/archive/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `turnofforder/archive/detail-edit-${this.row.id}-process`},processPanel)
            }else{
