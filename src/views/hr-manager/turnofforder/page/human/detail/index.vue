				...
				<el-row style="padding-left:10px;margin-top:20px;margin-bottom:20px;background-color: #F3F3F3">
					<table>
						<tr>
							...
							<td>
								<div style="width:140px">
									<el-form-item prop="workshift" style="margin-bottom: 0px;">
										<el-radio-group v-model="form.workshift" disabled>
							...
							<td style="width: 100%;">
								<el-input type="textarea" rows=4 disabled v-model="form.workinfo" placeholder="交接情况" maxlength=100></el-input>
							</td>
						</tr>
					</table>
				</el-row>
				<el-row style="padding-left:10px;margin-bottom:20px;background-color: #F3F3F3">
					<table>
						<tr>
							...
							<td>
								<div style="width:140px">
									<el-form-item prop="deviceshift" style="margin-bottom: 0px;">
										<el-radio-group :disabled="deviceshiftDisabled" v-model="form.deviceshift">
											<el-radio label="true">通过</el-radio>
											<br/>
											<el-radio label="false">未通过</el-radio>
										</el-radio-group>
							...
							<td class="gzjj-td-cls">
								<div style="width:360px">
									<el-form-item label="处理时间" prop="devicetime" label-width="120px">
										<el-date-picker value-format="yyyy-MM-dd" :disabled="devicetimeDisabled" v-model="form.devicetime" class="base-input-cls" placeholder="工作截止时间"></el-date-picker>
									</el-form-item>
								</div>
							</td>
							<td style="width:16px;padding: 0px 10px 0px 10px">
							...
							<td style="width: 100%;">
								<el-input type="textarea" rows=4 v-model="form.deviceinfo" :disabled="deviceinfoDisabled" placeholder="情况说明" maxlength=100></el-input>
							</td>
						</tr>
					</table>
				</el-row>
...
export default {
	name: 'turnofforder-human-edit-detail',
	...
	data(){
		return {
			...
			deviceshiftDisabled: false,
			devicetimeDisabled: false,
			deviceinfoDisabled: false,
			lastFormDetail: {},
			saveConurl: 'hr/turnofforder/human/saveHuman',
			submitConurl: 'hr/turnofforder/human/submitHuman',
			rejectConurl: 'hr/turnofforder/human/rejectHuman',
			authorityConurl: 'user/getModuleAccess',
			authorityParams: {moduleid: this.moduleid,token: this.$store.getters['user/getToken']},
			detailConurl: 'hr/turnofforder/human/getDetail',
			...
			rules: {
				deviceshift: [
					{ required: true, message: '请选择归还状况', trigger: 'blur' }
				],
				devicetime: [
	...
	mounted(){
		...
		setTimeout(() => {
			...
			this.$api[this.authorityConurl](this.authorityParams).then(resData=>{
				...
				this.deviceshiftDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.devicetimeDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
				this.deviceinfoDisabled = (resData.data === 1 || resData.data === 0) ? true : false;
			})
		}, 0);
	},
	methods: {
		readDetail(){
			if(this.process.processcode!=null){
                ...
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `turnofforder/human/detail-edit-${this.row.id}-process`},processPanel)
            }else{
