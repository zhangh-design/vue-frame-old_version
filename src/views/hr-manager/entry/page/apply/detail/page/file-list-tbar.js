...
const Tbar = {
    ...
    data(){
        return {
            defaultBtns: [
                ...
                action: `${ajaxBaseURL}/hr/entry/apply/${this.getPanel.mainPanel.formParams.process.id}/accessor`,
                data:{
    ...
    methods: {
        ...
        doDown(){
            ...
            let processID = this.getPanel.mainPanel.formParams.process.id
            // let detailID = this.getPanel.mainPanel.formParams.id
            /* 
            var a = document.createElement("a");
            a.setAttribute("style","display:none;");
            a.href = `${ajaxBaseURL}/hr/entry/apply/${processID}/accessor?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click();
             */
            window.open(`${ajaxBaseURL}/hr/entry/apply/${processID}/accessor?name=${encodeURIComponent(fileName)}&token=${this.$store.getters['user/getToken']}`, '_self');
        },
        doDelete(){
            if(!this.getPanel.getStore().length){
                this.$message({type: 'warning',duration:1000,message: '请先上传文件!'});
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                let processID = this.getPanel.mainPanel.formParams.process.id
                let ID = this.getPanel.mainPanel.formParams.id
                this.$api['hr/entry/apply/deleteUserFile']({processid:processID,name: fileName,token: this.$store.getters['user/getToken']}).then(resData=>{
