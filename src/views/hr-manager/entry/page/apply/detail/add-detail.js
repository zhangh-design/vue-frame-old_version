...
const Detail = {
    ...
    data(){
        return {
            ...
            processID: null,
            buttonsLayout: 'top',
            codeLink:`link-hrentry-code-${this._uid}`,
    ...
    methods: {
        ...
        initDetailData(){
            this.detailData = [
                {span: 1,name: 'code',type: 'TextField',width: 200,label: '员工编号',disabled:true,link:this.codeLink},
        ...
        confirmFn(value){
            ...
            this.$api[this.submitConurl]({...this.formParams,...this.formDetail}).then(resData=>{
                ...
                if(resData.code===CONST_DEFAULT_CONFIG.ajaxSuccesscode){
                    ...
                    !!this.window && this.window.close()
                    this.getLinkComponent('link-content-panel').getTabContent().removeTab(`hr/entry/apply/detail-add`)
                }
            })
        },
        doSubmit(){
