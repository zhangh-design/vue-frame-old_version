...
const Tbar = {
    ...
    data(){
        return {
            defaultBtns: [
                {text: '添加',name:'add',authority: ['write'],disabled:this.getPanel.curType==='edit'?false:true,listeners: {click: this.doAdd}},
                {text: '删除',name:'delete',authority: ['write'],link:'',disabled:this.getPanel.curType==='edit'?false:true,listeners: {click: this.doDelete}},
            ],
            userStyle: {
    ...
    mounted() {
        console.log(this.getPanel.moduleid)
    },
    methods: {
        doAdd(){
        ...
        doDelete(){
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                this.$api['hr/entry/apply/deleteDetail']({processid:processID,id: detailID,token: this.$store.getters['user/getToken']}).then(resData=>{
