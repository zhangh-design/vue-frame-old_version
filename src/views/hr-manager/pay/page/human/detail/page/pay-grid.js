...
const PayGrid = {
	...
	data() {
	  return {
			conurl: 'hr/payorder/human/readPayList',
			queryParams: {
	...
	mounted(){
		this.$nextTick(() => {
			this.$api['hr/payorder/human/getHumanDetail']({token: this.$store.getters['user/getToken'],processid: this.row.process.id}).then(resData=>{
