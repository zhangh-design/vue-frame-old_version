...
const Detail = {
    ...
    data(){
        return {
            conurl: 'hr/payorder/leader/doSubmitLeader',
            rejectConurl: 'hr/payorder/leader/doRejectLeader',
            detailConurl: 'hr/payorder/leader/getLeaderDetail',
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/payorder/leader',
                    dataconurl: 'hr/payorder/leader/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `hr/pay/leader/detail-edit-${this.record.id}-process`},processPanel)
            }else{
