...
const Detail = {
    ...
    data(){
        return {
            ...
            detailConurl: 'hr/payorder/apply/getApplyDetail',
            ...
            personinfoLink:`link-hrpay-userinfo-${this._uid}`,
    ...
    created(){
        this.$on(this.events.afterFormRender,()=>{
            apply(this.formParams,{payid: this.record.payid,deptcode: this.record.deptcode,deptname: this.record.deptname,personid: this.record.personid,personname: this.record.personname,personcode: this.record.personcode})
    ...
    methods: {
        ...
        setSelectProjectinfo(row){
            this.getLinkComponent(this.personinfoLink).setTextValue(row.name)
            this.getLinkComponent(this.userLink).setTextValue(row.id)
        },
        initDetailData(){
            this.detailData = [
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'hr/payorder/apply',
                    dataconurl: 'hr/payorder/apply/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `hr/pay/apply/detail-edit-${this.record.id}-process`},processPanel)
            }else{
