...
const PayGrid = {
	...
	data() {
	  return {
			conurl: 'hr/payorder/apply/readPayList',
			queryParams: {
				token: this.$store.getters['user/getToken'],
            	// personid: !!this.row ? this.row.personid: '',
				personid: '',
			},
			selMode: 'simple',
			isReloadGrid: false,
	...
	mounted(){
		this.$nextTick(() => {
			if(!(!!this.row)) return
			this.$api['hr/payorder/apply/getApplyDetail']({token: this.$store.getters['user/getToken'],processid: this.row.process.id}).then(resData=>{
