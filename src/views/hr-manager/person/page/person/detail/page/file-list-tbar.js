    ...
    data(){
        return {
            defaultBtns: [ 
                ...
                action: `${ajaxBaseURL}/hr/person/${this.getPanel.mainPanel.formParams.id}/accessor`,
                data:{
    ...
    methods: {
        ...
        doDelete(){
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                let processID = this.getPanel.mainPanel.formParams.id
                this.$api['hr/person/person/deleteFile']({id:processID,name: fileName,token: this.$store.getters['user/getToken']}).then(resData=>{
        ...
        doDown(){
            ...
            a.href = `${ajaxBaseURL}hr/person/${processID}/accessor?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click(); */
            window.open(`${ajaxBaseURL}hr/person/${processID}/accessor?name=${encodeURIComponent(fileName)}&token=${this.$store.getters['user/getToken']}`, '_self');
        }
    }
}
export default Tbar