...
const Detail = {
    ...
    data(){
        return {
            conurl: 'project/invoice/confirm/doSubmitConfirm',
            rejectConurl: 'project/invoice/confirm/doRejectCnfirm',
            detailConurl: 'project/invoice/confirm/getConfirmDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'project/invoice/confirm',
                    dataconurl: 'project/invoice/confirm/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `invoice/confirm/detail-edit-${this.record.id}-process`},processPanel)
            }else{
