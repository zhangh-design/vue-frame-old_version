...
const Detail = {
    ...
    data() {
        return {
            conurl: 'project/setup/charge/doSubmitCharge',
            rejectConurl: 'project/setup/charge/doRejectCharge',
            detailConurl: 'project/setup/charge/getChargeDetail',
            queryParams: {
    ...
    methods: {
        initDetailData() {
            this.detailData = [
                ...
                {span: 1,name: 'contractname',type: 'TextField',width: 200,label: '关联合同',disabled: true,emptyText:'请先选取关联合同'},
                {span: 1,name: 'contractid',type: 'TextHidden',width: 200},
            ]
        },
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: '/project/setup/charge',
                    dataconurl: 'project/setup/charge/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `setup/charge/detail-edit-${this.record.id}-process`},processPanel)
            }else{
