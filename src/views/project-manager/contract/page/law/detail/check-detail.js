...
const Detail = {
    ...
    data(){
        return {
            conurl: 'project/contract/law/doSubmitLaw',
            rejectConurl: 'project/contract/law/doRejectLaw',
            detailConurl: 'project/contract/law/getLawDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'project/contract/law',
                    dataconurl: 'project/contract/law/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `contract/law/detail-edit-${this.record.id}-process`},processPanel)
            }else{
