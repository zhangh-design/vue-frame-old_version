...
const Tbar = {
    ...
    data(){
        return {
            defaultBtns: [
                ...
                action: `${ajaxBaseURL}project/contract/archive/${this.getPanel.mainPanel.record.process.id}/accessor`,
                data:{
    ...
    methods: {
        ...
        doDown(){
            ...
            a.href = `${ajaxBaseURL}/project/contract/archive/${processID}/accessor?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click(); */
            window.open(`${ajaxBaseURL}/project/contract/archive/${processID}/accessor?name=${encodeURIComponent(fileName)}&token=${this.$store.getters['user/getToken']}`, '_self');
        },
        doDelete(){
            if(!this.getPanel.getStore().length){
                this.$message({type: 'warning',duration:1000,message: '请先上传文件!'});
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                let fileName = this.getPanel.getSelectedRow()['name']
                let processID = this.getPanel.mainPanel.record.process.id
                this.$api['project/contract/archive/deleteContractFile']({processid:processID,name: fileName,token: this.$store.getters['user/getToken']}).then(resData=>{
