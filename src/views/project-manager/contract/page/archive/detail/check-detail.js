...
const Detail = {
    ...
    data(){
        return {
            conurl: 'project/contract/archive/doSubmitArchive',
            rejectConurl: 'project/contract/archive/doRejectArchive',
            detailConurl: 'project/contract/archive/getArchiveDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'project/contract/archive',
                    dataconurl: 'project/contract/archive/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `contract/archive/detail-edit-${this.record.id}-process`},processPanel)
            }else{
