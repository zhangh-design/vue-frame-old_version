...
const Tbar = {
    ...
    data(){
        return {
            defaultBtns: [
                ...
                action: `${ajaxBaseURL}/project/contract/apply/${this.getPanel.mainPanel.processID}/accessor`,
                data:{
    ...
    methods: {
        ...
        doDown(){
            ...
            a.href = `${ajaxBaseURL}/project/contract/apply/${processID}/accessor?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click(); */
            window.open(`${ajaxBaseURL}/project/contract/apply/${processID}/accessor?name=${fileName}&token=${this.$store.getters['user/getToken']}`, '_self');
        },
        doDelete(){
            if(!this.getPanel.getStore().length){
                this.$message({type: 'warning',duration:1000,message: '请先上传文件!'});
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                let processID = this.getPanel.mainPanel.processID
                this.$api['project/contract/apply/deleteContractFile']({processid:processID,name: fileName,token: this.$store.getters['user/getToken']}).then(resData=>{
