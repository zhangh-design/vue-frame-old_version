...
const Detail = {
    ...
    data(){
        return {
            conurl: 'project/contract/manager/doSubmitManager',
            rejectConurl: 'project/contract/manager/doRejectManager',
            detailConurl: 'project/contract/manager/getManagerDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'project/contract/manager',
                    dataconurl: 'project/contract/manager/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `contract/manager/detail-edit-${this.record.id}-process`},processPanel)
            }else{
