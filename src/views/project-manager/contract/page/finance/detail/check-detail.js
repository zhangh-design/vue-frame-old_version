...
const Detail = {
    ...
    data(){
        return {
            conurl: 'project/contract/finance/doSubmitFinance',
            rejectConurl: 'project/contract/finance/doRejectFinance',
            detailConurl: 'project/contract/finance/getFinanceDetail',
            queryParams: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'project/contract/finance',
                    dataconurl: 'project/contract/finance/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `contract/finance/detail-edit-${this.record.id}-process`},processPanel)
            }else{
