...
const Detail = {
    ...
    data(){
        return {
            conurl: 'project/tender/dispose/doSubmitDispose',
            rejectConurl: 'project/tender/dispose/doRejectDispose',
            detailConurl: 'project/tender/dispose/getDisposeDetail',
            queryParams: {
    ...
    methods: {
        ...
        readDetail(){
            if(this.record.process.processcode!=null){
                ...
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `tender/dispose/detail-edit-${this.record.id}-process`},processdetail)
            }else{
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'project/tender/dispose',
                    dataconurl: 'project/tender/dispose/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `tender/dispose/detail-edit-${this.record.id}-process`},processPanel)
            }else{
                this.$message({type: 'success', message: '当前步骤没有流程!'});
            }
        },
