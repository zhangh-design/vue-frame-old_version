...
const Detail = {
    ...
    methods: {
        ...
        initDetailData(){
            ...
            this.detailData = [
               // {span: 1,name: 'code',type: 'TextField',width: 350,label: '合同编号',readonly: true},
                // {span: 1,name: 'projectname',type: 'TextField',width: 350,label: '项目名称',rule: [
                //     {required: true,message: '请输入项目名称', trigger: 'blur'},
                // ]},
                {span: 1,name: 'deptname',type: 'TextField',width: 350,label: '部门名称',readonly: true},
                //   {span: 1,name: 'customerunit',label: '客户单位',returnType: 'string',displayField: 'name',valueField: 'code',type: 'ComboBox',conurl:'project/common/contract/customers',queryParams: {token: this.$store.getters['user/getToken']},width: 200},
                    ...
                    this.$prompt('请输入意见', '提示', {
                        ...
                        this.$api[this.conurl]({...this.formParams,...this.formDetail}).then(resData=>{
                            ...
                            if(resData.code===CONST_DEFAULT_CONFIG.ajaxSuccesscode){
                                this.mainGrid.reloadGrid();
                                !!this.window && this.window.close()
                                this.getLinkComponent('link-content-panel').getTabContent().removeTab(`workload/superior/detail-edit-${this.formParams.id}`)
                            }
                            this.doResult(resData)
                        })
                    }).catch(() => {});
