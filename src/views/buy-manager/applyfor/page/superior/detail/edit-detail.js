...
const Detail = {
    ...
    data() {
        return {
            conurl: 'buy/applyfor/superior/doSubmitSuperior',
            saveConurl: 'buy/applyfor/superior/doSaveSuperior',
            rejectConurl: 'buy/applyfor/superior/doRejectSuperior',
            detailConurl: 'buy/applyfor/superior/getDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/applyfor/superior',
                    dataconurl: `buy/applyfor/superior/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/applyfor/superior/detail-edit-${this.record.id}-process`},processPanel)
            }else{
