...
const Detail = {
    ...
    data() {
        return {
            conurl: 'buy/applyfor/leader/doSubmitLeader',
            rejectConurl: 'buy/applyfor/leader/doRejectLeader',
            detailConurl: 'buy/applyfor/leader/getDetail',
            queryParams: {
    ...
    methods: {
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                const url=this.detailConurl.substring(0,this.detailConurl.lastIndexOf("/"));
                let processPanel = new processDetail
                processPanel.defaults = {
                    imgconurl: 'buy/applyfor/leader',
                    dataconurl: `buy/applyfor/leader/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/applyfor/leader/detail-edit-${this.record.id}-process`},processPanel)
            }else{
        ...
        initDetailData() {
            this.detailData = [
                ...
                {span: 2, name: 'remark', type: 'TextField', label: '采购说明', disabled: true},
            ]
        },
        confirmFn( value ){
