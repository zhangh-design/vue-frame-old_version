...
const Detail = {
    ...
    data() {
        return {
            conurl: 'buy/applyfor/charge/doSubmitCharge',
            rejectConurl: 'buy/applyfor/charge/doRejectCharge',
            detailConurl: 'buy/applyfor/charge/getDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/applyfor/charge',
                    dataconurl: `buy/applyfor/charge/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/applyfor/charge/detail-edit-${this.record.id}-process`},processPanel)
            }else{
