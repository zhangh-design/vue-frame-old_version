...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/payment/leader/doSubmitLeader',
            rejectConurl: 'buy/payment/leader/doRejectLeader',
            detailConurl: 'buy/payment/leader/getDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/payment/leader',
                    dataconurl: `buy/payment/leader/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/payment/leader/detail-edit-${this.record.id}-process`},processPanel)
            }else{
