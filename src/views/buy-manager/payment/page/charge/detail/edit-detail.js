...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/payment/charge/doSubmitCharge',
            rejectConurl: 'buy/payment/charge/doRejectCharge',
            detailConurl: 'buy/payment/charge/getDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/payment/charge',
                    dataconurl: `buy/payment/charge/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/payment/charge/detail-edit-${this.record.id}-process`},processPanel)
            }else{
