...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/payment/superior/doSubmitSuperior',
            rejectConurl: 'buy/payment/superior/doRejectSuperior',
            detailConurl: 'buy/payment/superior/getDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/payment/superior',
                    dataconurl: `buy/payment/superior/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/payment/superior/detail-edit-${this.record.id}-process`},processPanel)
            }else{
