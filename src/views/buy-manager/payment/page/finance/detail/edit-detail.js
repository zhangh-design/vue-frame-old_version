...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/payment/finance/doSubmitFinance',
            rejectConurl: 'buy/payment/finance/doRejectFinance',
            detailConurl: 'buy/payment/finance/getDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/payment/finance',
                    dataconurl: `buy/payment/finance/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/payment/finance/detail-edit-${this.record.id}-process`},processPanel)
            }else{
