...
const Tbar = {
    ...
    methods: {
        doDown(){
            ...
            a.href = `${ajaxBaseURL}buy/payment/manager/${processID}/payaccessory?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click(); */
            window.open(`${ajaxBaseURL}buy/payment/manager/${processID}/payaccessory?name=${encodeURIComponent(fileName)}&token=${this.$store.getters['user/getToken']}`, '_self');
        }
    }
}
export default Tbar