...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/payment/teller/doSubmitTeller',
            saveConurl: 'buy/payment/teller/doSaveTeller',
            rejectConurl: 'buy/payment/teller/doRejectTeller',
            detailConurl: 'buy/payment/teller/getDetail',
            queryParams: {
    ...
    methods: {
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/payment/teller',
                    dataconurl: `buy/payment/teller/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/payment/teller/detail-edit-${this.record.id}-process`},processPanel)
            }else{
        ...
        initDetailData(){
            this.detailData = [
                ...
                {span: 1,name: 'amount',type: 'TextField',width: 350,label: '实际付款金额(万)',rule:[
                    {required: true,message: '请输入实际付款金额(万)', trigger: 'blur'},
                    {validator: this.checkPaymentNum, trigger: 'blur'}
                ]},
                {span: 1,name: 'paytime',type: 'DateTimePicker',width: 350,label: '付款时间',rule:[
                ...
                {span: 1,name: 'payBtn',type: 'Button',label:'付款附件',text:'上传付款附件',listeners: {
                ...
                {span: 1,name: 'payaccessory',type: 'TextHidden',width: 350},
            ]
        },
