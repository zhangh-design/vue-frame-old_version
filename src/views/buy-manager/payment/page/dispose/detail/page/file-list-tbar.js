...
const Tbar = {
    ...
    data(){
        return {
            defaultBtns: [
                ...
                action: `${ajaxBaseURL}/buy/payment/dispose/${this.getPanel.mainPanel.record.process.id}/ticketaccessory`,
                data:{
    ...
    methods: {
        ...
        doDown(){
            ...
            a.href = `${ajaxBaseURL}/buy/payment/dispose/${processID}/ticketaccessory?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click();
        },
        doDelete(){
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                let processID = this.getPanel.mainPanel.record.process.id
                this.$api['buy/payment/dispose/deleteDisposeFile']({processid:processID,name: fileName,token: this.$store.getters['user/getToken']}).then(resData=>{
