...
const Tbar = {
    ...
    data(){
        return {
            defaultBtns:[
                ...
                action: `${ajaxBaseURL}/buy/orders/confirm/${this.getPanel.mainPanel.record.process.id}/invoice`,
                data:{
    ...
    methods: {
        ...
        doDown(){
            ...
            a.href = `${ajaxBaseURL}/buy/orders/confirm/${processID}/invoice?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click(); */
            window.open(`${ajaxBaseURL}/buy/orders/confirm/${processID}/invoice?name=${encodeURIComponent(fileName)}&token=${this.$store.getters['user/getToken']}`, '_self');
        },
        doDelete(){
            if(!this.getPanel.getStore().length){
                this.$message({type: 'warning',duration:1000,message: '请先上传文件!'});
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                this.$api['buy/orders/confirm/deleteInvoiceFile']({processid:processID,name: fileName,token: this.$store.getters['user/getToken']}).then(resData=>{
