...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/orders/charge/doSubmitCharge',
            rejectConurl: 'buy/orders/charge/doRejectCharge',
            detailConurl: 'buy/orders/charge/getChargeDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/orders/charge',
                    dataconurl: 'buy/orders/charge/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/orders/charge/detail-edit-${this.record.id}-process`},processPanel)
            }else{
