...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/orders/manager/doSubmitManager',
            rejectConurl: 'buy/orders/manager/doRejectManager',
            detailConurl: 'buy/orders/manager/getManagerDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/orders/manager',
                    dataconurl: 'buy/orders/manager/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/orders/manager/detail-edit-${this.record.id}-process`},processPanel)
            }else{
