...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/orders/dispose/doSubmitDispose',
            saveConurl:'buy/orders/dispose/doSaveDispose',
           // rejectConurl: 'buy/orders/dispose/doRejectDispose',
            detailConurl: 'buy/orders/dispose/getDisposeDetail',
            queryParams: {
    ...
    methods: {
        ...
        initDetailData(){
            this.detailData = [
                ...
                {span: 2,name: 'payamount',type: 'TextField',width: 140,labelWidth:'110px',label: '实际金额(元)',rule:[
                    {required: true,message: '请输入实际金额(元)', trigger: 'blur'},
                    {validator: this.checkPaymentNum, trigger: 'blur'}
                ]},
                ...
                {span: 1,name: 'payinfo',type: 'TextArea',width: 180,rows: 4,label: '报销信息'},
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/orders/dispose',
                    dataconurl: 'buy/orders/dispose/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/orders/dispose/detail-edit-${this.record.id}-process`},processPanel)
            }else{
