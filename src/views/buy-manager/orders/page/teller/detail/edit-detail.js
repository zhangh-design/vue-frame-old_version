...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/orders/teller/doSubmitTeller',
            saveConurl: 'buy/orders/teller/doSaveTeller',
            detailConurl: 'buy/orders/teller/getTellerDetail',
            queryParams: {
    ...
    methods: {
        ...
        initDetailData(){
            this.detailData = [
                ...
                {span: 1,name: 'paytime',type: 'DateTimePicker',width: 180,label: '打款时间',rule:[
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/orders/teller',
                    dataconurl: 'buy/orders/teller/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/orders/teller/detail-edit-${this.record.id}-process`},processPanel)
            }else{
