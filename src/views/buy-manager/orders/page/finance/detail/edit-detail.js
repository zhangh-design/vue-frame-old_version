...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/orders/finance/doSubmitFinance',
            rejectConurl: 'buy/orders/finance/doRejectFinance',
            detailConurl: 'buy/orders/finance/getFinanceDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/orders/finance',
                    dataconurl: 'buy/orders/finance/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/orders/finance/detail-edit-${this.record.id}-process`},processPanel)
            }else{
