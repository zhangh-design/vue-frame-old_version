...
const Tbar = {
    ...
    methods: {
        doDown(){
            ...
            a.href = `${ajaxBaseURL}/buy/orders/apply/${processID}/paycerts?name=${fileName}&token=${this.$store.getters['user/getToken']}`;
            a.download = "download";
            a.click(); */
            window.open(`${ajaxBaseURL}/buy/orders/apply/${processID}/paycerts?name=${encodeURIComponent(fileName)}&token=${this.$store.getters['user/getToken']}`, '_self');
        }
    }
}
export default Tbar