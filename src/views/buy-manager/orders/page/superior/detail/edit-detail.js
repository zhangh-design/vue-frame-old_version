...
const Detail = {
    ...
    data(){
        return {
            conurl: 'buy/orders/superior/doSubmitSuperior',
            rejectConurl: 'buy/orders/superior/doRejectSuperior',
            detailConurl: 'buy/orders/superior/getSuperiorDetail',
            queryParams: {
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/orders/superior',
                    dataconurl: 'buy/orders/superior/doStep',
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/orders/superior/detail-edit-${this.record.id}-process`},processPanel)
            }else{
