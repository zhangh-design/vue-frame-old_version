...
const Detail = {
    ...
    data() {
        return {
            conurl: 'buy/contract/leader/doSubmitLeader',
            rejectConurl: 'buy/contract/leader/doRejectLeader',
            detailConurl: 'buy/contract/leader/getDetail',
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/contract/leader',
                    dataconurl: `buy/contract/leader/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/contract/leader/detail-edit-${this.record.id}-process`},processPanel)
            }else{
