...
const Detail = {
    ...
    data() {
        return {
            conurl: 'buy/contract/charge/doSubmitCharge',
            rejectConurl: 'buy/contract/charge/doRejectCharge',
            detailConurl: 'buy/contract/charge/getDetail',
        ...
        doProcessDetail(){
            if(this.record.process.processcode!=null){
                ...
                processPanel.defaults = {
                    imgconurl: 'buy/contract/charge',
                    dataconurl: `buy/contract/charge/doStep`,
                    queryParams: {
                        token: this.$store.getters['user/getToken'],
                        processid: this.record.process.id,
                    }
                }
                this.getLinkComponent(`link-content-panel`).getTabContent().addTab({title: '流程记录',name: `buy/contract/charge/detail-edit-${this.record.id}-process`},processPanel)
            }else{
